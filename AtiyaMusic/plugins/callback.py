...
from config import (AUTO_DOWNLOADS_CLEAR, BANNED_USERS,
                    ...
                    BOT_NAME, adminlist)
from AtiyaMusic import YouTube, app
from AtiyaMusic.core.call import AtiyaMusic
from AtiyaMusic.misc import SUDOERS, db
from AtiyaMusic.utils import bot_sys_stats
from AtiyaMusic.utils.database import (
    get_active_chats,
    get_lang,
    is_active_chat,
    is_music_playing,
...
from AtiyaMusic.utils.decorators.language import languageCB
from AtiyaMusic.utils.formatters import seconds_to_min
from AtiyaMusic.utils.inline import (
    stream_markup,
    stream_markup_timer,
    telegram_markup,
...
from AtiyaMusic.utils.stream.autoclear import auto_clean
from AtiyaMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await CallbackQuery.answer()
        await AtiyaMusic.stop_stream(chat_id)
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await AtiyaMusic.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await AtiyaMusic.skip_stream(chat_id, link, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await AtiyaMusic.skip_stream(chat_id, queued, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
